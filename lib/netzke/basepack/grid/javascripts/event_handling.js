{
  ...
  onColumnResize: function(ct, cl, width){
    ...
    if (ct.itemId == 'normalHeaderCt') {
      index += ct.ownerCt.ownerCt.lockedGrid.headerCt.getColumnCount();
      this.onRefresh();
    } 
    this.resizeColumn({
      index: index,
  ...
  onColumnMove: function(ct, cl, oldIndex, newIndex){
    // when moving a column forwards, the new index appears to be 1 too high; a bug in Ext JS? Fixing.
     if (ct.itemId == 'normalHeaderCt') {
      newIndex += ct.ownerCt.ownerCt.lockedGrid.headerCt.getColumnCount();
      oldIndex += ct.ownerCt.ownerCt.lockedGrid.headerCt.getColumnCount();
      // this.onRefresh();
    } 
